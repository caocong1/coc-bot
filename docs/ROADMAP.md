...
| `.r` | 掷骰（含多轮、奖惩骰、暗骰） | ✅ |
| `.rd` | 叙事掷骰（附带描述文案） | ✅ |
| `.ra` / `.rc` | CoC7 技能检定（含角色卡自动查值、分类错误提示） | ✅ |
| `.sc` | 理智检定 | ✅ |
| `.ti` / `.li` | 疯狂症状 | ✅ |
| `.coc` | 人物作成 | ✅ |
| `.st` | 属性录入 | ✅ |
...
- [x] AI 错误分类：`DashScopeApiError` 区分配额耗尽 vs 临时错误，意图分类失败日志按类型分级
- [x] 信息节奏控制：每次回复最多推进 1 个调查节点，检定门控信息不提前泄露
- [x] 引号=对话 / 无引号=行动或内心活动规则
- [x] NPC 知识边界：NPC 不读取 PC 元数据，PC 自我介绍前不知道其职业
- [x] 职业适配剧情切入点
- [x] RP 与检定平衡：好 RP 可降难度/给奖励骰，日常社交好 RP 可免检定
- [x] KP 引导信息隔离：模组备注/条件句不直接给玩家
- [x] 全中文输出约束
