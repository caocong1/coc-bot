...
export class TokenStore {
  ...
  /** 为指定 QQ 号生成新 token（同时作废旧 token） */
  generate(qqId: number): string {
    // 清除该用户旧 token
    ...
    this.db.run(
      'INSERT INTO player_tokens (token, qq_id, expires_at, created_at) VALUES (?, ?, ?, ?)',
      [token, qqId, expiresAt.toISOString(), now.toISOString()],
    );

    return token;
  }

  /** 验证 token，返回对应 QQ 号；无效/过期返回 null */
  verify(token: string): number | null {
    const row = this.db.query<{ qq_id: number; expires_at: string }, string>(
      'SELECT qq_id, expires_at FROM player_tokens WHERE token = ?',
    ).get(token);
